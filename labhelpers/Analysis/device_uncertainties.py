...
class DeviceUncertainties:
    ...
    def thorlabs_s120vc_err(self, power, err_type, wvl=None):
        ...
        elif err_type.lower() == 'calibration':
            if wvl is None:
    ...
    def thorlabs_s121c_err(self, power, err_type, wvl=None):
        if err_type.lower() == 'calibration' and wvl < 400 / self.ureg.nanometer:
            raise ValueError("Wavelength too short. Minimum wavelength is 400 nm.")
        else:
            return self.thorlabs_s120vc_err(power, err_type, wvl=wvl)

    def thorlabs_s310c_err(self, power, err_type, wvl=None):
        if err_type.lower() == 'linearity':
            return 1 / 100 * power
        elif err_type.lower() == 'calibration':
            if wvl is None:
                ...
                if wvl < 190 * self.ureg.nanometer or wvl > 25 * self.ureg.micrometer:
                    raise ValueError("Wavelength out of range (0.19 - 25 µm)")
                elif wvl == 1064:
                    return 3 / 100 * power
                else:
                    return 5 / 100 * power
